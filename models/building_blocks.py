...
class AttributeAttentionEncoder(nn.Module):
    def __init__(self, x_dim, y_dim, z_dim, h_dim,
        ...
        self.pre_model = BaseMLP(x_dim + y_dim, h_dim, h_dim, num_layers=pre_num_layers-2)
        self.attention = MultiheadAttention(h_dim, h_dim, h_dim, h_dim)
        self.post_model = BaseMLP(y_dim * h_dim, z_dim*2, h_dim, num_layers=post_num_layers)
    ...
    def forward(self, x, y, mask=None):
        ...
        h = self.pre_model(h)
        q, k, v = h[..., -1:, :], h[..., :-1, :], h[..., :-1, :]
        h = self.attention(q, k, v, mask=mask).flatten(start_dim=-2).mean(dim=-2)
